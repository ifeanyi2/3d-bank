...
if(isset($_SESSION['uid'])){
  ...
  if(isset($_POST['fund']) &&  isset($_POST['type'])  &&  $_POST['type'] == 'Credit Fund'){
    ...
    VALUES ('$token', '$type', '$amount', '$txndate', '$description', '$to', '$account','$status','$description')";
...
has been credited with the sum of '.  $currency.''.$amount.' </p> <div style="margin-top: 25px;">
...
Copyright ©2021 '.$name.'. </p> </div> </div> </div>';
...
if($mail->send()) {
   ...
   $msg = "Account funded successfully!";
                ...
                $msg = "transaction successful, email notification not delivered due to localhost!";
            }
    ...
    if(isset($_POST['fund']) &&  isset($_POST['type'])  &&  $_POST['type'] == 'Debit Fund'){
      ...
      $token ='1234567890';
        $token = str_shuffle($token);
         ...
         $sql1 = "INSERT INTO tbl_transaction (tx_no, tx_type, amount, date, description, to_accno, account, status,comments) VALUES ('$token', '$type', '$amount', '$txndate', '$description', '$to', '$account','$status','$description')";
...
has been debited with the sum of '.  $currency.''.$amount.' </p> <div style="margin-top: 25px;">

<p>In case you need any further clarification for the same, please do get in touch with your Branch.</p> </div> </div> <div class="be_footer">
<div style="border-bottom: 1px solid #ccc;"></div> <p> Please do not reply to this email. Emails sent to this address will not be answered. 
Copyright ©2021 '.$account.'. </p> </div> </div> </div>';
...
<div class="be_logo" style="float: left;"> <img src="https://'.$bankurl.'/nonsms-bank-script/dashboard/admin/pages/logo/'.$logo.'"></div>
...
if($mail->send()) {
    ...
    $msg =  "Account UnVerified Successfully !";
      ...
      if(isset($_POST['acone'])){
        ...
        if (mysqli_query($link, $sql1)) {
            $msg = "Transfer With Codes activated and Direct Transfer Deactivated successfully!";
        } else {
            $msg = "Cannot activate Transfer With Codes! ";
        }
        }
        ...
        if(isset($_POST['actwo'])){
          ...
          if (mysqli_query($link, $sql1)) {
              $msg = "Direct Transfer activated and Transfer With Codes Deactivated successfully!";
          } else {
              $msg = "Cannot activate Direct Transfer! ";
          }
          }
...
if(mysqli_num_rows($result) > 0){
  ...
  if(isset($row['acone'])  && $row['acone']==1){
    $aca = 'Transfer With Codes Activated &nbsp;&nbsp;<i style="color:green;font-size:20px;" class="fa  fa-check" ></i>';
    
  }else{
    $aca = 'Transfer With Codes Deactivated   &nbsp;&nbsp;<i style="color:red;font-size:20px;" class="fa  fa-times" ></i>';
  }


  if(isset($row['actwo'])  && $row['actwo']==1){
    $acb = 'Direct Transfer Activated &nbsp;&nbsp;<i style="color:green;font-size:20px;" class="fa  fa-check" ></i>';
    
  }else{
    $acb = 'Direct Transfer Deactivated  &nbsp;&nbsp;<i style="color:red;font-size:20px;" class="fa  fa-times" ></i>';
  }
                ...
                <div class="row">
                  <div class="col-lg-12">
                    ...
                    <?php if($msg != "") echo "<div style='padding:20px;background-color:#dce8f7;color:black'> $msg</div class='btn btn-success'>" ."</br></br>";  ?>
                      ...
                      <div class="invoice-number"><img src="../../users/uploads/<?php echo $pics ;?>" width="150px" height="150px"/></div>
                    </div>
...
<button name="acone" class="btn btn-primary btn-icon icon-left"><i class="fas fa-check"></i> Activate Transfer With Codes / <i class="fas fa-times"></i> Deactivate Direct Transfer</button>
</form>
</br>
<form action="user-details.php?email=<?php echo $email ;?>" method="POST">
...
<button name="actwo" class="btn btn-success btn-icon icon-left"><i class="fas fa-check"></i> Activate Direct Transfer / <i class="fas fa-times"></i>Deactivate Transfer With Codes</button>
</form>
  ...
  color:black;
                    ...
                    <div class="table-responsive">
                      <table class="table table-striped table-hover table-md">
                        ...
                        <tr>
                        <th>IPN CODE</th>
                          <th class="text-center">Transfer With Codes Status</th>
                          <th class="text-center">IMF CODE</th>
                          <th class="text-right">Direct Transfer Status</th>
                        </tr>
                        <tr>
