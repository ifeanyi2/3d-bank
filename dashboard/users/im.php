...
$fname =  $row['fname'];
            ...
            $sql = "INSERT INTO tbl_transaction (tx_no, tx_type, amount, date, description, to_accno, account, status,  comments) 
            VALUES ('$tokens', 'debit', '$amount', '$date', '$desc', '$tacctno', '$acctno', 'SUCCESS', '$comments')";
          
            if(mysqli_query($link,$sql)){

            
  $link->query("UPDATE `tbl_users` SET `otp_hash`= ' ', balance = balance - $amount WHERE id = '$user_id'");
...
<div class="be_body" style="padding: 20px;"> <p style="line-height: 25px;"> 
             ...
             Dear ' . $row["fname"] . ' A Debit transaction of  '.  $currency.''.$amount.' occured in your account on  '.$date.' 
